...
for v in MICA_DEB_REPO_ROOT MICA_DEB_ARCH MICA_DEB_PRODUCER; do
    [ -n "${!v:-}" ] || { echo "error: ${v} is not set. This script is deb/kernel-virt-arm64-proof/producer.env's PREPARE hook and is run by build-env/deb/build.sh, which sets it; it is not a standalone command" >&2; exit 1; }
done
[ "${PREFLIGHT}" != 0 ] || [ -n "${MICA_DEB_STAGE:-}" ] || { echo "error: MICA_DEB_STAGE is not set. This script is deb/kernel-virt-arm64-proof/producer.env's PREPARE hook and is run by build-env/deb/build.sh, which sets it" >&2; exit 1; }

REPO_ROOT="${MICA_DEB_REPO_ROOT}"
STAGE="${MICA_DEB_STAGE:-}"
OUT="${REPO_ROOT}/_out/virt-arm64-proof"
BOARD="virt-arm64-proof"
...
present "${REPO_ROOT}/virt-arm64-proof/board.env" "the board definition" || true
present "${REPO_ROOT}/virt-arm64-proof/manifests/board.pkgs" "the board package manifest" || true
...
install -m 0644 "${REPO_ROOT}/virt-arm64-proof/board.env" "${STAGE}/board.env"
[ ! -f "${REPO_ROOT}/virt-arm64-proof/evidence.json" ] || install -m 0644 "${REPO_ROOT}/virt-arm64-proof/evidence.json" "${STAGE}/evidence.json"
...
firmware="$(sed -n 's/^BOARD_FIRMWARE_FILES="\{0,1\}\([^"]*\)"\{0,1\}$/\1/p' "${REPO_ROOT}/virt-arm64-proof/board.env" | head -1)"
if [ -n "${firmware}" ]; then
    mkdir -p "${STAGE}/firmware"
    for f in ${firmware}; do
        name="${f#/usr/lib/firmware/}"
        install -m 0644 "${REPO_ROOT}/virt-arm64-proof/bsp/rootfs/firmware/${name}" "${STAGE}/firmware/${name}"
    done
fi
[ ! -f "${REPO_ROOT}/virt-arm64-proof/bsp/component-copyright" ] || install -m 0644 "${REPO_ROOT}/virt-arm64-proof/bsp/component-copyright" "${STAGE}/component-copyright"
...
stage_tree "${REPO_ROOT}/virt-arm64-proof/manifests" "${STAGE}/manifests"
