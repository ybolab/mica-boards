...
- Products: every board package the boards declared plus `mica-kernel-<board>`, which packs the BSP outputs (`_out/<board>`), `board.env`, `evidence.json`, `manifests/`, the support image's firmware and the verity trust certificate the kernel embeds under `/usr/lib/mica/board/<board>/`; published together as the GitHub Release `build-<commit12>` of `ybolab/mica-boards`; the assembly (`mica-build`) imports them through `deps/packages/` and reads each kernel archive into `_out/boards/<board>/`
