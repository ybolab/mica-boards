...
# Scratch under the repository's _out, not the board's: a board directory is data.
mkdir -p "$PWD/../../_out"
...
run() {
    : > "$RK_LOG"
    make -s flash-mos RKDEVELOPTOOL="$work/rkdeveloptool" MICA_IMAGE="$1" > "$work/output" 2>&1
}
