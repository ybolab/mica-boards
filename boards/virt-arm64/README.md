...
The virt-arm64 board (UEFI on QEMU virt) of Mica OS: the board definition
(`board.env`), the BSP build (`bsp/`), the board packages (`deb/`) and the
board evidence (`evidence.json`). One board of `ybolab/mica-boards`; `make virt-arm64-kernel`, `make pool` and
...
assembly's signing workspace or a copy of its certificate). `mica-kernel-virt-arm64`
